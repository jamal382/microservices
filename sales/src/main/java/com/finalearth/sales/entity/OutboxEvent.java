 ...
 *
 * <p><strong>The problem it exists to solve.</strong> Placing an order means doing two
 * things: writing the order to Postgres and publishing {@code order.placed} to Kafka.
 * Those are two different systems, and there is no transaction spanning both. Write the
 * database first and the process can die before the publish, leaving an order that nothing
 * downstream will ever act on. Publish first and the process can die before the commit,
 * leaving {@code inventory} reserving stock for an order that does not exist. This is the
 * dual-write problem, and no ordering of the two calls fixes it.
 *
 ...
 */
@Entity
@Table(name = "outbox", schema = "sales")
