package com.finalearth.payment.entity;
 ...
 *
 * <p><strong>The problem it exists to solve.</strong> Charging a card means doing two
 * things: writing the payment row to Postgres and publishing {@code payment.completed} to
 * Kafka. Those are two different systems, and there is no transaction spanning both. Write
 * the database first and the process can die before the publish, leaving a customer charged
 * for an order that never reaches {@code CONFIRMED}. Publish first and the process can die
 * before the commit, confirming an order against a payment this service has no record of.
 * This is the dual-write problem, and it is the worst version of it in the system, because
 * one side of the inconsistency is somebody's money.
 *
